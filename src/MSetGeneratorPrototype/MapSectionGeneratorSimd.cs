...
namespace MSetGeneratorPrototype
{
	public class MapSectionGeneratorSimd
	{
		public MapSectionResponse GenerateMapSection(MapSectionRequest mapSectionRequest)
		{
			...
			var startingCx = ScalarMathHelper.CreateSmx(mapPosition.X, fixedPointFormat);
			var startingCy = ScalarMathHelper.CreateSmx(mapPosition.Y, fixedPointFormat);
			var delta = ScalarMathHelper.CreateSmx(samplePointDelta.Width, fixedPointFormat);
			...
			for (int j = 0; j < samplePointsY.Length; j++)
			{
				...
				var vecMath = new VecMath9(fixedPointFormat, stride, threshold);
