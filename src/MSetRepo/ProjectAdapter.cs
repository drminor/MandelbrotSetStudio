	...
	public class ProjectAdapter : IProjectAdapter
	{
		...
		public bool TryGetProject(string name, [MaybeNullWhen(false)] out Project project)
		{
			...
			if (projectReaderWriter.TryGet(name, out var projectRecord))
			{
				...
				var lookupColorMapByTargetIteration = JobOwnerHelper.CreateLookupColorMapByTargetIteration(targetIterationColorMapRecs);
		...
		public ColorBandSetInfo? GetColorBandSetInfo(ObjectId id)
		{
			...
			if (cbsRecord != null)
			{
				var result = new ColorBandSetInfo(cbsRecord.Id, cbsRecord.Name ?? cbsRecord.ColorBandsSerialNumber.ToString(), cbsRecord.Description, cbsRecord.LastAccessed, cbsRecord.ColorBandsSerialNumber, cbsRecord.ColorBandRecords.Length, cbsRecord.ColorBandRecords.Max(y => y.CutOff));
				return result;
			}
			else
			{
		...
		private ColorBandSet? GetColorBandSet(Job job, ColorBandSetReaderWriter colorBandSetReaderWriter, IDictionary<ObjectId, ColorBandSet>? colorBandSetCache, out bool isCacheHit)
		{
			...
			else
			{
				...
				else
				{
					...
					if (colorBandSet.ProjectId != job.OwnerId)
					{
		...
		private Poster BuildPoster(PosterRecord target, ColorBandSetReaderWriter colorBandSetReaderWriter)
		{
			...
			var lookupColorMapByTargetIteration = JobOwnerHelper.CreateLookupColorMapByTargetIteration(jobs, colorBandSets, new Dictionary<int, TargetIterationColorMapRecord>());
