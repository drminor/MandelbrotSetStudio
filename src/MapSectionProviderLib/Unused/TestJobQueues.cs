...
{
	public class TestJobQueues
	{
		static void Main()
		{

			int priorityCount = 7;
			JobQueues queue = new JobQueues();
			var bc = new BlockingCollection<KeyValuePair<int, MapSectionGenerateRequest>>(queue, 50);
			...
			tasks[0] = Task.Run(() =>
			{
				...
				while (!cts.Token.IsCancellationRequested && itemsToAdd-- > 0)
				{
					...
					var wr = new MapSectionWorkRequest(count++, new MapSectionRequest(), WorkAction);

					var item = new KeyValuePair<int, MapSectionGenerateRequest>(priority, new MapSectionGenerateRequest(count++, wr, GenWorkAction));
			...
			tasks[1] = Task.Run(() =>
			{
				while (!bc.IsCompleted && !cts.Token.IsCancellationRequested)
				{
					...
					//KeyValuePair<int, MapSectionWorkRequest> item = new KeyValuePair<int, MapSectionWorkRequest>();
					bool success = false;
					success = bc.TryTake(out var item);
					if (success)
					{
		...
		private static void WorkAction(MapSectionRequest msq, MapSection ms)
		{

		}

		private static void GenWorkAction(MapSectionWorkRequest mswr, MapSectionResponse msr)
		{
