...
#pragma warning disable 0809  //warning CS0809: Obsolete member 'SpanExp<T>.Equals(object)' overrides non-obsolete member 'object.Equals(object)'
...
namespace System
{
	...
	[NonVersionable]
	public readonly ref struct SpanExp<T>
	{
		...
		/// </exception>
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public SpanExp(T[]? array, int start, int length)
		{
			if (array == null)
			{
				if (start != 0 || length != 0)
			...
			if (default(T)! == null && array.GetType() != typeof(T[])) // TODO-NULLABLE: default(T) == null warning (https://github.com/dotnet/roslyn/issues/34757)
				ThrowHelper.ThrowArrayTypeMismatchException();
#if BIT64
            // See comment in SpanExp<T>.Slice for how this works.
		...
		[CLSCompliant(false)]
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public unsafe SpanExp(void* pointer, int length)
		{
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		internal SpanExp(ref T ptr, int length)
		{
		...
		/// </summary>
		public static bool operator !=(SpanExp<T> left, SpanExp<T> right) => !(left == right);
		...
		/// </summary>
		public static implicit operator SpanExp<T>(T[]? array) => new SpanExp<T>(array);

		/// <summary>
		/// Defines an implicit conversion of a <see cref="ArraySegment{T}"/> to a <see cref="Span{T}"/>
		/// </summary>
		public static implicit operator SpanExp<T>(ArraySegment<T> segment) =>
			new SpanExp<T>(segment.Array, segment.Offset, segment.Count);

		/// <summary>
		/// Returns an empty <see cref="Span{T}"/>
		/// </summary>
		public static SpanExp<T> Empty => default;
		...
		public ref struct Enumerator
		{
			...
			private readonly SpanExp<T> _span;
			...
			[MethodImpl(MethodImplOptions.AggressiveInlining)]
			internal Enumerator(SpanExp<T> span)
			{
				_span = span;
				_index = -1;
			}
		...
		/// </exception>
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public void CopyTo(SpanExp<T> destination)
		{
		...
		public bool TryCopyTo(SpanExp<T> destination)
		{
			bool retVal = false;
			if ((uint)_length <= (uint)destination.Length)
			{
		...
		/// this does *not* check to see if the *contents* are equal.
		/// </summary>
		public static bool operator ==(SpanExp<T> left, SpanExp<T> right) =>
		...
		/// </summary>
		public static implicit operator ReadOnlySpanExp<T>(SpanExp<T> span) =>
			new ReadOnlySpanExp<T>(ref span._pointer.Value, span._length);
		...
		/// </exception>
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public SpanExp<T> Slice(int start)
		{
			if ((uint)start > (uint)_length)
				ThrowHelper.ThrowArgumentOutOfRangeException();

			return new SpanExp<T>(ref Unsafe.Add(ref _pointer.Value, start), _length - start);
		}
		...
		/// </exception>
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public SpanExp<T> Slice(int start, int length)
		{
#if BIT64
			...
			return new SpanExp<T>(ref Unsafe.Add(ref _pointer.Value, start), length);
		}
