...
namespace Test
{
	public unsafe struct NewRational : IComparable<NewRational>, IEquatable<NewRational>
	{
		int sign; uint[] bits;
		...
		{
			return obj is NewRational && Equals((NewRational)obj);
		}
		public bool Equals(NewRational b)
		{
		...
		public int CompareTo(NewRational b)
		{
			return cmp(this, b);
		}
		...
		public static NewRational Parse(string s)
		{
			NewRational a = 0, b = a, e = 1, f = 10;
		...
		public static NewRational operator -(NewRational v)
		{
			if (v.sign != 0) v.sign ^= 1 << 31; return v;
		}
		public static implicit operator NewRational(int v)
		{
			var a = (uint)(v < 0 ? -v : v);
			if (a <= 0x7fff) { NewRational r; r.sign = (int)(a << 16) | (v & (1 << 31)); r.bits = null; return r; }
			return (Tmp)v;
		}
		public static implicit operator NewRational(float v)
		{
			return (Tmp)v;
		}
		public static implicit operator NewRational(double v)
		{
			return (Tmp)v;
		}
		public static implicit operator NewRational(decimal v)
		{
			return (Tmp)v;
		}
		public static explicit operator double(NewRational v)
		{
			double r;
		...
		public static explicit operator float(NewRational v)
		{
			return (float)(double)v;
		}
		public static explicit operator long(NewRational v)
		{
			long r;
		...
		public static Tmp operator +(NewRational a, NewRational b)
		{
			fixed (uint* pa = a.bits, pb = b.bits) { rat ca, cb; a.fix(&ca, pa); b.fix(&cb, pb); return add(&ca, &cb); }
		}
		public static Tmp operator +(NewRational a, Tmp b)
		{
			fixed (uint* p = a.bits) { rat c; a.fix(&c, p); return add(&c, b.p); }
		}
		public static Tmp operator +(Tmp a, NewRational b)
		{
		...
		public struct Tmp
		{
			...
			public override string ToString() { return ((NewRational)this).ToString(); }
			...
			public static bool operator <(Tmp a, Tmp b) { return cmp(a, b) < 0; }
			public static bool operator >(Tmp a, Tmp b) { return cmp(a, b) > 0; }
			public static implicit operator NewRational(Tmp v)
			{
				NewRational r; var a = &v.p->num; var b = &v.p->den;
			...
			for (; ; )
			{
				...
				if (min == 1)
				{
					if (max != 1)
					{
						...
						a.n = 1; if (u == 0) { a.p[0] = b.p[0]; break; }
						a.p[0] = (uint)u;
					}
		...
		static big mul(big a, big b, uint* p)
		{
			...
			for (int i = a.n, k; i-- != 0;)
			{
				...
				for (k = 0; k < b.n; k++)
				{
					var t = (ulong)b.p[k] * a.p[i] + r.p[i + k] + c;
					r.p[i + k] = (uint)t;
					c = (uint)(t >> 32);
				}
		...
		static int cmp(NewRational a, NewRational b)
		{
		...
		internal struct big
		{
			...
			public override string ToString()
			{
				...
				Tmp t; t.p = &c; return ((NewRational)t).ToString();
			}
		}
		internal struct rat
		{
